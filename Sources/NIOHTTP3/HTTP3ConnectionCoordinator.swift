...
final class HTTP3ConnectionCoordinator<QUICStreamCreator: NIOQUICHelpers.QUICStreamCreator> {
    ...
    private func createQPACKEncoderInstructionStream() {
        ...
        self.createOutboundUnidirectionalStream(ofType: .qpackEncoder) { parameter -> EventLoopFuture<Void> in
            let streamChannel = parameter.channel
            let streamID = parameter.streamID

            self.qpackCoder!.outboundEncoderStreamReady(
                OutboundQPACKEncoderChannel(
                    channel: streamChannel,
                    preferHuffmanEncoding: self.preferHuffmanEncoding
                )
            )
            streamChannel.closeFuture.assumeIsolated().whenComplete { result in
                self.onStreamClosed(streamID: streamID, seenEOF: true, streamType: .unidirectional(.qpackEncoder))
            }
    ...
    private func createQPACKDecoderInstructionStream() {
        self.eventLoop.assertInEventLoop()
        self.createOutboundUnidirectionalStream(ofType: .qpackDecoder) { parameter -> EventLoopFuture<Void> in
            let streamChannel = parameter.channel
            let streamID = parameter.streamID

            self.qpackCoder!.outboundDecoderStreamReady(
                OutboundQPACKDecoderChannel(channel: streamChannel)
            )
            streamChannel.closeFuture.assumeIsolated().whenComplete { result in
                self.onStreamClosed(streamID: streamID, seenEOF: true, streamType: .unidirectional(.qpackEncoder))
            }
    ...
    func receivedControlFrame(_ frame: HTTP3Frame, streamID: QUICStreamID) {
        ...
        case .onSettings(let onSettings):
            if case .settings(let settings) = frame {
                #warning("TODO: effective dynamic table size should be min from init value and the new")
                self.qpackCoder!.receivedRemoteSettings(
                    maxQueueSize: Int(clamping: settings.settings.qpackBlockedStreams),
                    effectiveDynamicTableSize: Int(clamping: settings.settings.qpackMaximumTableCapacity)
                )
            }
...
extension HTTP3ConnectionCoordinator: ConnectionDelegate {
